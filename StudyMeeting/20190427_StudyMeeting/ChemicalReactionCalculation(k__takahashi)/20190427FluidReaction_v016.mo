package FluidReaction
  ...
  equation
    ...
    port_b.p = port_a.p;
    -mXi_b[1] / Medium.data[1].MM = mXi_a[1] / Medium.data[1].MM + mXi_a[3] / Medium.data[3].MM;
//H2O
    -mXi_b[2] / Medium.data[2].MM = mXi_a[2] / Medium.data[2].MM - 0.5 * mXi_a[3] / Medium.data[3].MM;
//O2
    -mXi_b[3] / Medium.data[3].MM = 0.0;
//H2
  ...
  model nvfCtl "Normal Volume Flow Controller"
    extends Modelica.Fluid.Interfaces.PartialTwoPort(allowFlowReversal = false);
    parameter Real fix_nv_flow(unit="Nm3/s",min=0.0)=0.0 "Fixed normal volume flow rate";
  protected
    Real m_flow(unit="kg/s");
  algorithm
    m_flow := 101325.0*fix_nv_flow/((inStream(port_a.Xi_outflow[:])*Medium.data[:].R)*273.15);
  equation
    0 = port_a.m_flow + port_b.m_flow;
    ...
    annotation(
      ...
      Icon(coordinateSystem(initialScale = 0.1), graphics = {Rectangle(origin = {92, -33}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-192, 73}, {8, -7}}), Text(origin = {0, -4},extent = {{-40, 40}, {40, -40}}, textString = "V"), Ellipse(origin = {-2, 30}, fillPattern = FillPattern.Solid, extent = {{-2, 2}, {6, -6}}, endAngle = 360), Text(origin = {-1, -42}, extent = {{-79, 2}, {81, -38}}, textString = "%fix_nv_flow Nm3/s")}));
  end nvfCtl;
  
  model mfCtl "Mass Flow Controller"
    extends Modelica.Fluid.Interfaces.PartialTwoPort(allowFlowReversal = false);
    parameter Real fix_m_flow(unit="kg/s") = 0.0 "Fixed mass flow rate";
  equation
    port_a.m_flow + port_b.m_flow = 0;
    port_a.m_flow = fix_m_flow;
    port_b.Xi_outflow[:] = inStream(port_a.Xi_outflow[:]);
    port_b.h_outflow = inStream(port_a.h_outflow);
    port_b.C_outflow = inStream(port_a.C_outflow);
    port_a.Xi_outflow[:] = inStream(port_b.Xi_outflow[:]);
    ...
    annotation(
      ...
      Icon(coordinateSystem(initialScale = 0.1), graphics = {Rectangle(origin = {92, -33}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-192, 73}, {8, -7}}), Text(extent = {{-40, 40}, {40, -40}}, textString = "m"), Ellipse(origin = {-2, 24}, fillPattern = FillPattern.Solid, extent = {{-2, 2}, {6, -6}}, endAngle = 360), Text(origin = {-1, -42}, extent = {{-59, 12}, {61, -38}}, textString = "%fix_m_flow kg/s")}));
  end mfCtl;

  model tCtl "Temperature controller"
    extends Modelica.Fluid.Interfaces.PartialTwoPort(allowFlowReversal = false);
    parameter Real fix_T(unit="K") = 298.15 "Fixed temperature";
    Modelica.SIunits.SpecificEnthalpy dh "Heat supply(+) / loss(-)";
  equation
    port_a.p = port_b.p;
    port_a.m_flow + port_b.m_flow = 0;
    port_b.Xi_outflow[:] = inStream(port_a.Xi_outflow[:]);
//  port_b.h_outflow = Medium.h_TX(T = fix_T, X = port_a.Xi_outflow, exclEnthForm=false,refChoice = Modelica.Media.Interfaces.Choices.ReferenceEnthalpy.ZeroAt25C);
    port_b.h_outflow = Medium.specificEnthalpy(Medium.setState_pTX(p = port_a.p, T = fix_T, X = inStream(port_a.Xi_outflow[:])));
    port_b.C_outflow = inStream(port_a.C_outflow);
    port_a.Xi_outflow[:] = inStream(port_b.Xi_outflow[:]);
    port_a.h_outflow = inStream(port_b.h_outflow);
    port_a.C_outflow = inStream(port_b.C_outflow);
    dh = port_b.h_outflow - port_a.h_outflow;
    annotation(
      Documentation(info = "<html>
  </html>"),
      Icon(coordinateSystem(initialScale = 0.1), graphics = {Rectangle(origin = {92, -33}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-192, 73}, {8, -7}}), Text(extent = {{-40, 40}, {40, -40}}, textString = "T"), Text(origin = {-14, -50}, extent = {{-46, 20}, {74, -30}}, textString = "%fix_T K")}));
  end tCtl;
  
  model VolToMass "Converter from volume flow rate to mass flow rate"
    replaceable package Medium = Modelica.Media.Interfaces.PartialMedium "Medium in the component"
        annotation (choicesAllMatching = true);   
    parameter Boolean use_v_flow_in = false "Get the volume flow rate from the input connector"
      annotation(Evaluate=true, HideResult=true, choices(checkBox=true));
    parameter Boolean use_T_in= false "Get the temperature from the input connector"
      annotation(Evaluate=true, HideResult=true, choices(checkBox=true));
    parameter Boolean use_Y_in = false "Get the mole fraction from the input connector"
      annotation(Evaluate=true, HideResult=true, choices(checkBox=true));
    parameter Boolean use_D_in = false "Get the trace substances per volume from the input connector"
      annotation(Evaluate=true, HideResult=true, choices(checkBox=true));
    parameter Real v_flow(unit="Nm3/s") = 0 "Fixed volume flow rate going out of the fluid port"
      annotation
       (Dialog(enable = not use_v_flow_in));
    parameter Real T(unit="K") = Medium.T_default "Fixed value of temperature"
      annotation
       (Dialog(enable = not use_T_in));
    parameter Real Y[Medium.nX](each unit="mol/mol") = Medium.X_default "Fixed value of mole fraction"
      annotation
       (Dialog(enable = (not use_Y_in) and Medium.nXi > 0));
    parameter Medium.ExtraProperty D[Medium.nC](quantity=Medium.extraPropertiesNames) =  Medium.C_default "Fixed values of trace substances per volume"
      annotation (Dialog(enable = (not use_D_in) and Medium.nC > 0));
    Modelica.Blocks.Interfaces.RealInput v_flow_in if use_v_flow_in "Prescribed volume flow rate"
      annotation
       (Placement(visible = true,transformation(extent = {{-120, 60}, {-80, 100}}, rotation = 0), iconTransformation(extent = {{-120, 60}, {-80, 100}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealInput T_in if use_T_in "Prescribed fluid temperature"
      annotation
       (Placement(transformation(extent={{-140,20},{-100,60}}), iconTransformation(extent={{-140,20},{-100,60}})));
    Modelica.Blocks.Interfaces.RealInput Y_in[Medium.nX] if use_Y_in "Prescribed mole fraction"
      annotation
       (Placement(visible = true, transformation(extent = {{-140, -60}, {-100, -20}}, rotation = 0), iconTransformation(extent = {{-140, -60}, {-100, -20}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealInput D_in[Medium.nC] if use_D_in "Prescribed trace substances per volume"
      annotation
      (Placement(visible = true, transformation(extent = {{-120, -100}, {-80, -60}}, rotation = 0), iconTransformation(extent = {{-120, -100}, {-80, -60}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealOutput m_flow_out "Mass flow rate" annotation(
        Placement(visible = true, transformation(origin = {100, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealOutput T_out "Temperature" annotation(
        Placement(visible = true, transformation(origin = {110, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealOutput X_out[Medium.nX] "Mass fraction" annotation(
        Placement(visible = true, transformation(origin = {110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealOutput C_out[Medium.nC] "Trace substances per mass" annotation(
        Placement(visible = true, transformation(origin = {100, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  protected
    Modelica.Blocks.Interfaces.RealInput v_flow_in_internal
      "Needed to connect to conditional connector";
    Modelica.Blocks.Interfaces.RealInput T_in_internal
      "Needed to connect to conditional connector";
    Modelica.Blocks.Interfaces.RealInput Y_in_internal[Medium.nX]
      "Needed to connect to conditional connector";
    Modelica.Blocks.Interfaces.RealInput D_in_internal[Medium.nC]
      "Needed to connect to conditional connector";   
  algorithm
    X_out[:] := Modelica.Media.Interfaces.PartialMixtureMedium.moleToMassFractions(Y_in_internal,Medium.data[:].MM);
    m_flow_out := 101325.0*v_flow_in_internal/((X_out[:]*Medium.data[:].R)*273.15);
    C_out[:] := D_in_internal[:]*v_flow_in_internal/m_flow_out;
    T_out := T_in_internal;
  equation
    connect(v_flow_in, v_flow_in_internal);
    connect(T_in, T_in_internal);
    connect(Y_in, Y_in_internal);
    connect(D_in, D_in_internal);
    ...
    if not use_D_in then
      D_in_internal = D;
    end if;
  annotation
  (Icon(graphics = {
    Rectangle(origin = {0, 20},fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 80}, {100, -120}}),
Text(origin = {-2, 46}, extent = {{-78, 14}, {82, -106}}, textString = "V->m"), Text(lineColor = {0, 0, 255}, extent = {{-150, 130}, {150, 170}}, textString = "%name"), Ellipse(origin = {-65, 33}, fillPattern = FillPattern.Solid, extent = {{1, -1}, {7, -7}}, endAngle = 360), Ellipse(origin = {57, 23}, fillPattern = FillPattern.Solid, extent = {{-1, 1}, {5, -5}}, endAngle = 360)},
    coordinateSystem(initialScale = 0.1)));
  end VolToMass;
  
  model emptyTwoPort "simple two port unit for developing other component"
    extends Modelica.Fluid.Interfaces.PartialTwoPort(allowFlowReversal = false);
  equation
    0 = port_a.m_flow + port_b.m_flow;
    port_a.Xi_outflow = inStream(port_b.Xi_outflow);
  ...
  model monitor "monitoring unit"
    ...
    Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium = Medium,
        ...
        Placement(visible = true, transformation(origin = {-20, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-26, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium = Medium,
                       m_flow(max=0)) annotation(
        Placement(visible = true, transformation(origin = {20, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {26, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));   
    Real mass_flow(unit="kg/s") "Mass flow rate";
    Real mol_flow(unit="mol/s") "mol flow rate";
    Real nv_flow(unit="Nm3/s") "Normal volume flow rate";
    Real v_flow(unit="m3/s") "Volume flow rate";
    Real X[Medium.nXi](each unit="kg/kg") "Mass fractions";
    Real Y[Medium.nXi](each unit="mol/mol") "Mole fraction";
    Real X_flow[Medium.nXi](each unit="kg-sp/s") "Mass flow rate of substances";
    Real Ym_flow[Medium.nXi](each unit="mol-sp/s") "Mole flow rate of substances";
    Real Ynv_flow[Medium.nXi](each unit="Nm3-sp/s") "Normal volume flow rate of substances";
    Real hjmass(unit="J/kg") "Specific enthalpy";
    Real hjmol(unit="J/mol") "Specific enthalpy";
    Real hjnv(unit="J/Nm3") "Specific enthalpy";
    Real hcmass(unit="cal/kg") "Specific enthalpy";
    Real hcmol(unit="cal/mol") "Specific enthalpy";
    Real hcnv(unit="cal/Nm3") "Specific enthalpy";
    Real hj_flow(unit="J/s") "Enthalpy flow rate";
    Real hc_flow(unit="cal/s") "Enthalpy flow rate";
    Real Tk(unit="K") "Temperature";
    Real Td(unit="degC") "Temperature";
  algorithm
    mass_flow := port_a.m_flow;
    X[:] := inStream(port_a.Xi_outflow);
    hjmass := inStream(port_a.h_outflow);
    Tk := Medium.temperature(Medium.setState_phX(port_a.p, inStream(port_a.h_outflow), inStream(port_a.Xi_outflow[:])));
    Y[:] := Modelica.Media.Interfaces.PartialMixtureMedium.massToMoleFractions(X[:],Medium.data[:].MM);
    mol_flow := mass_flow/(Y[:]*Medium.data[:].MM);
    nv_flow := mass_flow*((X[:]*Medium.data[:].R)*273.15)/101325.0;
    v_flow := mass_flow*((X[:]*Medium.data[:].R)*Tk)/port_a.p;
    X_flow[:] := X[:]*mass_flow;
    Ym_flow[:] := X_flow[:]./Medium.data[:].MM;
    Ynv_flow[:] := X_flow[:].*Medium.data[:].R*273.15/101325.0;
    hjmol := hjmass*(Y[:]*Medium.data[:].MM);
    hjnv := hjmass*101325.0/((X[:]*Medium.data[:].R)*Tk);
    hcmass := hjmass/4.184;
    hcmol := hjmol/4.184;
    hcnv := hjnv/4.184;
    hj_flow := hjmass*mass_flow;
    hc_flow := hj_flow/4.184;
    Td := Tk-273.15;
  equation
    port_a.p=port_b.p;
    0 = port_a.m_flow + port_b.m_flow;
    ...
    port_a.h_outflow = inStream(port_b.h_outflow);
  annotation(
        Icon(graphics = {Rectangle(fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-20, 20}, {20, -20}}), Text(origin = {-12, 16}, extent = {{-8, 4}, {32, -36}}, textString = "M")}, coordinateSystem(initialScale = 0.1)));
  end monitor;


  package MyGas "H2,O2,H2O"
    extends Modelica.Media.IdealGases.Common.MixtureGasNasa(mediumName = "MyGas", data = {SingleGasesData.H2O, SingleGasesData.O2, SingleGasesData.H2}, fluidConstants = {FluidData.H2O, FluidData.O2, FluidData.H2}, substanceNames = {"H2O", "O2", "H2"}, reference_X = {0.4, 0.2, 0.4}, excludeEnthalpyOfFormation = false, referenceChoice = Modelica.Media.Interfaces.Choices.ReferenceEnthalpy.ZeroAt25C);
    annotation(
      Documentation(info = "<html>
  ...
  model Gascomb4
    ...
    Modelica.Fluid.Sensors.Temperature temperature(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {50, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Vessels.ClosedVolume volume(redeclare package Medium = Medium, T_start = 298.15, V = 0.1, X_start = {0.0, 1.0, 0.0}, nPorts = 2, use_T_start = true, use_portsData = false) annotation(
      Placement(visible = true, transformation(origin = {2, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(volume.ports[2], pipeout.port_a) annotation(
      Line(points = {{2, 30}, {4, 30}, {4, 20}, {24, 20}, {24, 20}, {24, 20}}, color = {0, 127, 255}));
    connect(simpleCombustor1.port_b, volume.ports[1]) annotation(
      Line(points = {{-16, 20}, {-2, 20}, {-2, 30}, {2, 30}, {2, 30}}, color = {0, 127, 255}));
    connect(pipeout.port_b, temperature.port) annotation(
      Line(points = {{44, 20}, {50, 20}}, color = {0, 127, 255}));
    connect(temperature.port, ambient.ports[1]) annotation(
      Line(points = {{50, 20}, {70, 20}}, color = {0, 127, 255}));
    connect(pipeH2.port_b, simpleCombustor1.port_a) annotation(
      Line(points = {{-44, 40}, {-40, 40}, {-40, 20}, {-36, 20}}, color = {0, 127, 255}));
    connect(pipeO2.port_b, simpleCombustor1.port_a) annotation(
      Line(points = {{-44, 0}, {-40, 0}, {-40, 20}, {-36, 20}}, color = {0, 127, 255}));
    connect(O2.ports[1], pipeO2.port_a) annotation(
      Line(points = {{-70, 0}, {-64, 0}}, color = {0, 127, 255}, thickness = 0.5));
    connect(H2.ports[1], pipeH2.port_a) annotation(
      Line(points = {{-70, 40}, {-64, 40}}, color = {0, 127, 255}, thickness = 0.5));
    annotation(
      experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02),
      __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"));
  end Gascomb4;
  ...
  model Gascomb5
    ...
    Modelica.Fluid.Pipes.StaticPipe pipeout(redeclare package Medium = Medium, allowFlowReversal = false, diameter = 0.01, length = 1) annotation(
      Placement(visible = true, transformation(origin = {26, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    FluidReaction.simpleVolCombustor0 simpleVolCombustor01(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {-10, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(pipeH2.port_b, simpleVolCombustor01.port_a) annotation(
      Line(points = {{-44, 40}, {-36, 40}, {-36, 20}, {-20, 20}}, color = {0, 127, 255}));
    connect(pipeO2.port_b, simpleVolCombustor01.port_a) annotation(
      Line(points = {{-44, 0}, {-35, 0}, {-35, 20}, {-20, 20}}, color = {0, 127, 255}));
    connect(pipeout.port_a, simpleVolCombustor01.port_b) annotation(
      Line(points = {{16, 20}, {0, 20}}, color = {0, 127, 255}));
    connect(pipeout.port_b, temperature.port) annotation(
      Line(points = {{36, 20}, {50, 20}, {50, 20}, {50, 20}}, color = {0, 127, 255}));
    connect(O2.ports[1], pipeO2.port_a) annotation(
      Line(points = {{-70, 0}, {-64, 0}}, color = {0, 127, 255}, thickness = 0.5));
    connect(H2.ports[1], pipeH2.port_a) annotation(
      Line(points = {{-70, 40}, {-64, 40}}, color = {0, 127, 255}, thickness = 0.5));
  ...
  model Gascomb6
    ...
    Modelica.Fluid.Sources.FixedBoundary ambient(redeclare package Medium = Medium, nPorts = 1, p = 101325) annotation(
      Placement(visible = true, transformation(origin = {80, 20}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    inner Modelica.Fluid.System system(T_ambient(displayUnit = "K") = 298.15, T_start(displayUnit = "K") = 298.15, p_ambient(displayUnit = "Pa") = 101325, p_start(displayUnit = "Pa") = 101325) annotation(
      Placement(visible = true, transformation(origin = {50, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sensors.Temperature temperature(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {50, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T O2(redeclare package Medium = Medium, T = 298.15, X = {0.0, 1.0, 0.0}, m_flow = 0.009, nPorts = 1) annotation(
    ...
    Modelica.Fluid.Pipes.StaticPipe pipeout(redeclare package Medium = Medium, allowFlowReversal = false, diameter = 0.01, length = 1) annotation(
      Placement(visible = true, transformation(origin = {26, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    simpleVolCombustor simpleVolCombustor1(redeclare package Medium = Medium, T_start = 298.15, V = 0.1, X_start = {0.0, 1.0, 0.0}, p_start = 101325) annotation(
      Placement(visible = true, transformation(origin = {-18, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(simpleVolCombustor1.port_b, pipeout.port_a) annotation(
      Line(points = {{-8, 20}, {16, 20}, {16, 20}, {16, 20}}, color = {0, 127, 255}));
    connect(pipeO2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-44, 0}, {-38, 0}, {-38, 20}, {-28, 20}, {-28, 20}, {-28, 20}}, color = {0, 127, 255}));
    connect(pipeH2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-44, 40}, {-38, 40}, {-38, 20}, {-28, 20}, {-28, 20}, {-28, 20}}, color = {0, 127, 255}));
    connect(pipeout.port_b, temperature.port) annotation(
      Line(points = {{36, 20}, {50, 20}, {50, 20}, {50, 20}}, color = {0, 127, 255}));
    connect(O2.ports[1], pipeO2.port_a) annotation(
      Line(points = {{-70, 0}, {-64, 0}}, color = {0, 127, 255}, thickness = 0.5));
  ...
  model Gascomb7
    ...
    FluidReaction.simpleVolCombustor simpleVolCombustor1(redeclare package Medium = Medium, T_start = 298.15, V = 0.1, X_start = {0.0, 1.0, 0.0}, p_start = 101325) annotation(
      Placement(visible = true, transformation(origin = {-6, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    FluidReaction.tCtl tCtl1(redeclare package Medium = Medium, fix_T(displayUnit = "K") = 298.15) annotation(
      Placement(visible = true, transformation(origin = {-12, 52}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    FluidReaction.mfCtl mfCtl1(redeclare package Medium = Medium, fix_m_flow = 0.01) annotation(
      Placement(visible = true, transformation(origin = {12, 52}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  equation
    connect(tCtl1.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-22, 52}, {-28, 52}, {-28, 20}, {-16, 20}, {-16, 20}}, color = {0, 127, 255}));
    connect(mfCtl1.port_b, tCtl1.port_a) annotation(
      Line(points = {{2, 52}, {-2, 52}, {-2, 52}, {-2, 52}}, color = {0, 127, 255}));
    connect(teeJunctionIdeal1.port_3, mfCtl1.port_a) annotation(
      Line(points = {{24, 30}, {24, 30}, {24, 52}, {22, 52}, {22, 52}}, color = {0, 127, 255}));
    connect(pipeH2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-44, 40}, {-38, 40}, {-38, 20}, {-16, 20}}, color = {0, 127, 255}));
    connect(pipeO2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-44, 0}, {-38, 0}, {-38, 20}, {-16, 20}}, color = {0, 127, 255}));
    connect(simpleVolCombustor1.port_b, teeJunctionIdeal1.port_1) annotation(
      Line(points = {{4, 20}, {14, 20}}, color = {0, 127, 255}));
    connect(teeJunctionIdeal1.port_2, pipeout.port_a) annotation(
      Line(points = {{34, 20}, {40, 20}}, color = {0, 127, 255}));
    connect(pipeout.port_b, temperature.port) annotation(
      Line(points = {{60, 20}, {68, 20}}, color = {0, 127, 255}));
    connect(temperature.port, ambient.ports[1]) annotation(
      Line(points = {{68, 20}, {80, 20}}, color = {0, 127, 255}));
    connect(O2.ports[1], pipeO2.port_a) annotation(
      Line(points = {{-70, 0}, {-64, 0}}, color = {0, 127, 255}, thickness = 0.5));
    connect(H2.ports[1], pipeH2.port_a) annotation(
      Line(points = {{-70, 40}, {-64, 40}}, color = {0, 127, 255}, thickness = 0.5));
  ...
  model Gascomb8
    replaceable package Medium = MyGas;
    Modelica.Fluid.Sources.MassFlowSource_T H2(redeclare package Medium = Medium, nPorts = 1, use_C_in = true, use_T_in = true, use_X_in = true, use_m_flow_in = true) annotation(
      Placement(visible = true, transformation(origin = {-54, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary ambient(redeclare package Medium = Medium, nPorts = 1, p = 101325) annotation(
    ...
    Modelica.Fluid.Sources.MassFlowSource_T O2(redeclare package Medium = Medium, nPorts = 1, use_C_in = true, use_T_in = true, use_X_in = true, use_m_flow_in = true) annotation(
      Placement(visible = true, transformation(origin = {-54, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Pipes.StaticPipe pipeH2(redeclare package Medium = Medium, allowFlowReversal = false, diameter = 0.01, length = 1) annotation(
  ...
  FluidReaction.VolToMass volToMass1(redeclare package Medium = Medium,T = 298.15, Y = {0.0, 0.0, 1.0}, v_flow = 0.4)  annotation(
      Placement(visible = true, transformation(origin = {-84, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  FluidReaction.VolToMass volToMass2(redeclare package Medium = Medium,T = 298.15, Y = {0.0, 1.0, 0.0}, v_flow = 0.6)  annotation(
      Placement(visible = true, transformation(origin = {-84, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  FluidReaction.nvfCtl nvfCtl1(redeclare package Medium = Medium,fix_nv_flow = 0.5)  annotation(
  ...
  Modelica.Fluid.Pipes.StaticPipe pipeO2 annotation(
      Placement(visible = true, transformation(origin = {-28, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(pipeO2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-18, 0}, {-16, 0}, {-16, 20}, {16, 20}}, color = {0, 127, 255}));
    connect(O2.ports[1], pipeO2.port_a) annotation(
      Line(points = {{-44, 0}, {-38, 0}}, color = {0, 127, 255}, thickness = 0.5));
    connect(monitor1.port_b, ambient.ports[1]) annotation(
      Line(points = {{78, -32}, {58, -32}, {58, -34}, {58, -34}}, color = {0, 127, 255}));
    connect(pipeout.port_b, monitor1.port_a) annotation(
      Line(points = {{92, 20}, {96, 20}, {96, -32}, {84, -32}, {84, -32}, {82, -32}}, color = {0, 127, 255}));
    connect(pipeH2.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-18, 40}, {-16, 40}, {-16, 20}, {16, 20}}, color = {0, 127, 255}));
    connect(volToMass2.m_flow_out, O2.m_flow_in) annotation(
      Line(points = {{-74, 8}, {-69, 8}, {-69, 10}, {-64, 10}, {-64, 8}, {-64, 8}}, color = {0, 0, 127}));
    connect(volToMass2.T_out, O2.T_in) annotation(
      Line(points = {{-73, 4}, {-69, 4}, {-69, 6}, {-65, 6}, {-65, 6}, {-67, 6}, {-67, 4}, {-67, 4}}, color = {0, 0, 127}));
    connect(volToMass2.X_out, O2.X_in) annotation(
      Line(points = {{-73, -4}, {-70, -4}, {-70, -2}, {-65, -2}, {-65, -2}, {-66, -2}, {-66, -4}, {-67, -4}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass2.C_out, O2.C_in) annotation(
      Line(points = {{-74, -8}, {-69, -8}, {-69, -6}, {-64, -6}, {-64, -8}, {-64, -8}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass1.m_flow_out, H2.m_flow_in) annotation(
      Line(points = {{-74, 48}, {-64, 48}, {-64, 48}, {-64, 48}}, color = {0, 0, 127}));
    connect(volToMass1.T_out, H2.T_in) annotation(
      Line(points = {{-73, 44}, {-67, 44}, {-67, 46}, {-67, 46}, {-67, 44}, {-67, 44}}, color = {0, 0, 127}));
    connect(volToMass1.X_out, H2.X_in) annotation(
      Line(points = {{-73, 36}, {-65, 36}, {-65, 36}, {-67, 36}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass1.C_out, H2.C_in) annotation(
      Line(points = {{-74, 32}, {-64, 32}, {-64, 32}, {-64, 32}}, color = {0, 0, 127}, thickness = 0.5));
    connect(H2.ports[1], pipeH2.port_a) annotation(
      Line(points = {{-44, 40}, {-38, 40}}, color = {0, 127, 255}, thickness = 0.5));
    connect(nvfCtl1.port_a, teeJunctionIdeal1.port_3) annotation(
      Line(points = {{54, 52}, {56, 52}, {56, 30}, {56, 30}, {56, 30}}, color = {0, 127, 255}));
    connect(tCtl1.port_a, nvfCtl1.port_b) annotation(
      Line(points = {{30, 52}, {34, 52}, {34, 52}, {34, 52}}, color = {0, 127, 255}));
    connect(simpleVolCombustor1.port_b, teeJunctionIdeal1.port_1) annotation(
      Line(points = {{36, 20}, {46, 20}}, color = {0, 127, 255}));
    connect(tCtl1.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{10, 52}, {6, 52}, {6, 20}, {16, 20}}, color = {0, 127, 255}));
    connect(teeJunctionIdeal1.port_2, pipeout.port_a) annotation(
      Line(points = {{66, 20}, {72, 20}}, color = {0, 127, 255}));
    annotation(
      experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02),
      __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"));
  end Gascomb8;
  ...
  model Gascomb9
    ...
    FluidReaction.simpleVolCombustor simpleVolCombustor1(redeclare package Medium = Medium, T_start = 298.15, V = 1.0, X_start = {0.0, 1.0, 0.0}, p_start = 101325) annotation(
      Placement(visible = true, transformation(origin = {10, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Fittings.TeeJunctionIdeal teeJunctionIdeal1(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {40, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    FluidReaction.tCtl tCtl1(redeclare package Medium = Medium, fix_T(displayUnit = "K") = 298.15) annotation(
  ...
  equation
    connect(teeJunctionIdeal1.port_2, monitor1.port_a) annotation(
      Line(points = {{50, 20}, {74, 20}, {74, -32}, {68, -32}, {68, -32}, {66, -32}}, color = {0, 127, 255}));
    connect(O2.ports[1], simpleVolCombustor1.port_a) annotation(
      Line(points = {{-24, 0}, {-20, 0}, {-20, 20}, {0, 20}, {0, 20}, {0, 20}}, color = {0, 127, 255}));
    connect(H2.ports[1], simpleVolCombustor1.port_a) annotation(
      Line(points = {{-24, 40}, {-20, 40}, {-20, 20}, {0, 20}, {0, 20}, {0, 20}}, color = {0, 127, 255}));
    connect(monitor1.port_b, ambient.ports[1]) annotation(
      Line(points = {{61.4, -32}, {55.4, -32}, {55.4, -32}, {49.4, -32}, {49.4, -32}, {43.4, -32}, {43.4, -34}, {41.4, -34}, {41.4, -34}, {41.4, -34}, {41.4, -34}, {41.4, -34}}, color = {0, 127, 255}));
    connect(tCtl1.port_a, nvfCtl1.port_b) annotation(
      Line(points = {{14, 52}, {18, 52}, {18, 52}, {18, 52}}, color = {0, 127, 255}));
    connect(nvfCtl1.port_a, teeJunctionIdeal1.port_3) annotation(
      Line(points = {{38, 52}, {39, 52}, {39, 52}, {40, 52}, {40, 30}, {40, 30}, {40, 30}, {40, 30}, {40, 30}}, color = {0, 127, 255}));
    connect(tCtl1.port_b, simpleVolCombustor1.port_a) annotation(
      Line(points = {{-6, 52}, {-10, 52}, {-10, 20}, {0, 20}}, color = {0, 127, 255}));
    connect(simpleVolCombustor1.port_b, teeJunctionIdeal1.port_1) annotation(
      Line(points = {{20, 20}, {30, 20}}, color = {0, 127, 255}));
    connect(volToMass2.C_out, O2.C_in) annotation(
      Line(points = {{-54, -8}, {-49, -8}, {-49, -6}, {-44, -6}, {-44, -8}, {-44, -8}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass2.X_out, O2.X_in) annotation(
      Line(points = {{-53, -4}, {-50, -4}, {-50, -2}, {-45, -2}, {-45, -2}, {-46, -2}, {-46, -4}, {-47, -4}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass2.T_out, O2.T_in) annotation(
      Line(points = {{-53, 4}, {-49, 4}, {-49, 6}, {-45, 6}, {-45, 6}, {-47, 6}, {-47, 4}, {-47, 4}}, color = {0, 0, 127}));
    connect(volToMass2.m_flow_out, O2.m_flow_in) annotation(
      Line(points = {{-54, 8}, {-49, 8}, {-49, 10}, {-44, 10}, {-44, 8}, {-44, 8}}, color = {0, 0, 127}));
    connect(volToMass1.C_out, H2.C_in) annotation(
      Line(points = {{-54, 32}, {-44, 32}, {-44, 32}, {-44, 32}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass1.X_out, H2.X_in) annotation(
      Line(points = {{-53, 36}, {-45, 36}, {-45, 36}, {-47, 36}}, color = {0, 0, 127}, thickness = 0.5));
    connect(volToMass1.T_out, H2.T_in) annotation(
      Line(points = {{-53, 44}, {-47, 44}, {-47, 46}, {-47, 46}, {-47, 44}, {-47, 44}}, color = {0, 0, 127}));
    connect(volToMass1.m_flow_out, H2.m_flow_in) annotation(
      Line(points = {{-54, 48}, {-44, 48}, {-44, 48}, {-44, 48}}, color = {0, 0, 127}));
    annotation(
      experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02),
      __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"));
  end Gascomb9;
  ...
  model MFCtest
    replaceable package Medium = MyGas;
    Modelica.Fluid.Sources.MassFlowSource_T H2(redeclare package Medium = Medium, T = 298.15, X = {0.0, 0.0, 1.0}, m_flow = 1, nPorts = 1, use_m_flow_in = false) annotation(
    ...
    Modelica.Fluid.Fittings.TeeJunctionIdeal teeJunctionIdeal1(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {-32, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary ambient2(redeclare package Medium = Medium, nPorts = 1, p = 101325) annotation(
      Placement(visible = true, transformation(origin = {30, 54}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    FluidReaction.mfCtl mfCtl1(redeclare package Medium = Medium, fix_m_flow = 0.1) annotation(
      Placement(visible = true, transformation(origin = {-10, 54}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(mfCtl1.port_b, ambient2.ports[1]) annotation(
      Line(points = {{0, 54}, {20, 54}, {20, 54}, {20, 54}}, color = {0, 127, 255}));
    connect(teeJunctionIdeal1.port_3, mfCtl1.port_a) annotation(
      Line(points = {{-32, 30}, {-32, 30}, {-32, 54}, {-20, 54}, {-20, 54}}, color = {0, 127, 255}));
    connect(teeJunctionIdeal1.port_2, ambient.ports[1]) annotation(
      Line(points = {{-22, 20}, {24, 20}, {24, 20}, {22, 20}, {22, 20}}, color = {0, 127, 255}));
    connect(H2.ports[1], teeJunctionIdeal1.port_1) annotation(
      Line(points = {{-66, 20}, {-42, 20}, {-42, 20}, {-42, 20}}, color = {0, 127, 255}));
    annotation(
      Diagram);
  end MFCtest;
  ...
  model GTCtest
    ...
    Modelica.Fluid.Sensors.Temperature temperature1(redeclare package Medium = Medium) annotation(
      Placement(visible = true, transformation(origin = {40, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    FluidReaction.tCtl tCtl1(redeclare package Medium = Medium, fix_T(displayUnit = "K")) annotation(
      Placement(visible = true, transformation(origin = {0, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(tCtl1.port_b, temperature1.port) annotation(
      Line(points = {{10, 20}, {40, 20}, {40, 20}, {40, 20}}, color = {0, 127, 255}));
    connect(temperature.port, tCtl1.port_a) annotation(
      Line(points = {{-40, 20}, {-10, 20}, {-10, 20}, {-10, 20}}, color = {0, 127, 255}));
    connect(temperature1.port, ambient.ports[1]) annotation(
      Line(points = {{40, 20}, {60, 20}, {60, 20}, {60, 20}}, color = {0, 127, 255}));
    connect(H2.ports[1], temperature.port) annotation(
